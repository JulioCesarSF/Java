	...
	public static void main(String[] args) {
		...
		try (Scanner t = new Scanner(System.in)) {
			...
			for (int i = 0; i < nomes.length; i++) {
				...
				System.out.println("Informe o slário do funcionário");
			...
			for (int i = 0; i < tempo.length; i++) {
				if (tempo[i] >= 5 && salario[i] < 780)
					System.out
							.println("Nome:\t" + nomes[i] + " - novo salário R$" + (salario[i] + (salario[i] * 0.35)));
				else if (tempo[i] >= 5)
					System.out
							.println("Nome:\t" + nomes[i] + " - novo salário R$" + (salario[i] + (salario[i] * 0.25)));
				else if (salario[i] < 780)
					System.out
							.println("Nome:\t" + nomes[i] + " - novo salário R$" + (salario[i] + (salario[i] * 0.15)));
			}
